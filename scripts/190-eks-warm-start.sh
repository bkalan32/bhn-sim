...
phase() { local n="$1"; shift; (( n >= FROM )) || return 1; PT=$(date +%s); step "Phase $n/8 · $*  ($(( (PT - T0) / 60 )) min elapsed)"; }
...
if phase 1 "Network — infra/aws/env (VPC, NAT, ECR)"; then
  ...
  else "$LAB_ROOT/scripts/152-aws-vpc.sh" plan && "$LAB_ROOT/scripts/152-aws-vpc.sh" apply; fi
  mark 1 "network"
fi
...
if phase 2 "Images — the bytes kind runs, pushed to ECR (Day 15's 153)"; then
  ...
  "$LAB_ROOT/scripts/153-aws-ecr-push.sh" && mark 2 "images"
fi

if phase 3 "Cluster — infra/aws/eks (~12 min; + the bot's Pod Identity role, Day 19)"; then
  "$LAB_ROOT/scripts/160-eks.sh" plan && "$LAB_ROOT/scripts/160-eks.sh" apply && "$LAB_ROOT/scripts/161-eks-kubeconfig.sh" && mark 3 "cluster"
fi

if phase 4 "Platform — infra/aws/platform (CRDs, kps with Day 18's routing, Tempo, Fluent Bit -> CloudWatch, gp3)"; then
  KUBE_CONTEXT=aws-lab "$LAB_ROOT/scripts/162-eks-platform.sh" plan && KUBE_CONTEXT=aws-lab "$LAB_ROOT/scripts/162-eks-platform.sh" apply && mark 4 "platform"
fi

if phase 5 "Services — k8s/aws rendered from k8s/ + alerts + dashboards + the ticket layer wired (163)"; then
  KUBE_CONTEXT=aws-lab "$LAB_ROOT/scripts/163-eks-deploy.sh" && mark 5 "services"
fi
