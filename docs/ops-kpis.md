...
| 0019-a | 18 | **pipeline Grafana annotations silently failing since Day 13** (`kps-grafana` secret gone with B10; `grafanaAnnotate` read the old name; every deploy/rollback since unrecorded — the bot's deploys collector and the remediator's tier-2 signature blind to real releases) | never — a WARNING line in green builds; read by a human on build #37 | 5 days | 1 min (the build log said it) | Jenkinsfile fix, same build | none; follow-up: alert on a missing annotation after a deploy-service build |
| 0019-b | 18 | **platform restart storm, caught by the new alert**: `PlatformPodRestarting` opened its first tickets within an hour of existing (`INC-1789231840-517b` 16:50Z: otel collector 6×/h — Day 14's 0017-c, still going — and the kps operator 3×; `INC-1789234779-6ada` 17:3xZ: 11 pods in the hour, scheduler and controller-manager 6× each: the Jenkins build + drill + Splunk squeezing the VM again, 0017-a's shape) | alert (`PlatformPodRestarting`, warning, service=platform) — the Day 14 ask, first day in service; the 17:42Z daily report carried it unprompted | 5 min (`for:`) | — (cause: VM load; otel's exit 2 still unexplained) | self-resolving as load drops | none — tier 3; follow-up: the otel exit capture (0017-c) and a `startupProbe`/limits pass on the kps operator |
| 0020 | 19 | **game day 2, fault 1: creeping latency, no errors** — `BASE_LATENCY_MS` 80→600 by `kubectl set env` outside the pipeline, on EKS — `INC-1789245416-72a4` | alert (`ActivationLatencyBudgetBurn`, **warning** → ticket, no page); 1h burn 17.2× at first look | **3 m 35 s** (not ~9: the 1h window was 30 min old on a fresh cluster) | bot +31 s: **wrong lead** ("slow issuer" from the 3 % baseline declines), medium, kb-001/002 correctly rejected; **copilot right at ≈20:46** (≈13 min from the fault, 10 from the alert): 18 hands, 65 s, `kubectl_get` → the env var and 8-min pods; the deploys collector said "none in 6h" (annotations only) | alert → resolved **16.0 min**; fix (20:47:50Z revert) → resolved 5 min | manual revert; remediator tier 3 (correct). Three collectors on EKS ✅ |
| 0021 | 19 | **game day 2, fault 2: settlement crash** (`SETTLEMENT_FAIL_MODE=crash`, `db_unreachable`), EKS — `INC-1789245706-ac0f` | alert (`SettlementZeroRecords` first — a crash pushes 0; `JobFailed` +2 m) | **3 m 40 s** | signature +16 s after `JobFailed`; bot hypothesis +29 s cited **kb-005 for a kb-004 fault — the KB's own discriminator was wrong** (fixed) | alert → resolved **24.0 min**: **AUTO** re-run 20:44:02 FAILED → **AUTO** retry 20:47:19 FAILED → cooldown → vendor reset 20:50:19 → cron clean 20:55:00 (fix → first clean run 5 min) → AUTO after cooldown 21:02:02 succeeded (on a healthy service — stale alert) → resolved 21:05:46 (the 900-s window over four failed jobs) | **auto** (tier 1) + vendor; **0 operator actions** |
| 0022 | 19 | **game day 2, fault 3: partner email degradation, 50 %** (`EMAIL_FAIL_RATE=0.5`), EKS — `INC-1789245911-ce45` | alert (`EgiftHighErrorRate`, critical) | **2 m 49 s** (INC-0016 at 35 %: 4 m 17 s) | bot +33 s: **kb-003 by id, high, "the email partner must recover … tier 3"** — right first try; decided by CloudWatch rows (`email_delivery_failed 82` vs 7) | alert → resolved **10.0 min** (0016: 10 m 27 s); fix → resolved 2 min; first human touch +7 min (behind the warning) | manual escalate (partner ticket as a note 20:52:12Z); remediator tier 3 (correct) |
| 0022-a | 19 | **the deploys collector is blind to changes made outside the pipeline** — third incident (0015 drift, 0016 hand edit, 0020 `set env`): "no deploys in 6h" on a ticket whose pods were 8 min old, repeated by the closing brief as "Deploys: none in 24h" after two real rollouts | the copilot's `kubectl_get`, not the collector | — | 1 min once asked | — | follow-up: read ReplicaSet/rollout history (template hash + age), annotate "rolled, no annotation" |
| 0022-b | 19 | **kb-004 misled the settlement hypothesis**: "a crash pushes nothing; compare kb-005 which pushes 0" — false since Day 8's push fix (a crash pushes `status=0`, `records=0` — the same pair a strict silent run pushes) | the retro (reading the hypothesis against the scenario) | — | — | fixed in code: kb-004 + kb-005 symptoms and `learned_from` — the alert pair is identical for crash and strict-silent; the job's last log line discriminates, and it is not in the hypothesis' input (follow-up) | manual (KB maintenance, Day 17's rule) |
| 0022-c | 19 | **the remediator re-ran a healthy settlement** at 21:02:02Z: cooldown expired while `SettlementJobFailed` still counted the old failed jobs; `detect` reads the alert, never the service (`settlement_last_success_timestamp` was 7 min fresh) | the retro (timeline) | — | — | harmless (Job Complete, 5 639 records) | follow-up: `detect` checks last-success recency → "already recovered, not acting" |
...
> **Week 3 (0018–0022).** The same platform on EKS: the dependency drill alerted in 2 m 48 s
> with two sources (0018), the knowledge base turned a medium-confidence hypothesis into a
> high one on the same fault (0019), a day of alert hygiene made *warning* mean "a ticket" and
> gave the platform a 250-word morning brief (0019-a/b), and game day 2 (0020–0022) ran three
> faults on a cluster that had existed for three hours: detection **3 m 35 s / 3 m 40 s /
> 2 m 49 s**, alert → ticket 15 s each, three sources on every cloud ticket for the first
> time, every ticket closed by the platform. The known fault (settlement) was handled with
> **zero operator actions** — two automated re-runs, a vendor reset, a clean run; the
> ambiguous one needed a person to overrule the hypothesis and ask the copilot (≈13 min to the
> cause); the external one needed a person to post the ticket that leaves the building. What
> did not improve: TTR is still the alert windows (fix → resolved 5 / 15.5 / 2 min); and the
> platform still cannot see a change made outside the pipeline — 0022-a is the third
> incident with that line, so it is the first backlog item of the first 90 days.
